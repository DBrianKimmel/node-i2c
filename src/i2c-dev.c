...
static ssize_t i2cdev_write(struct file *file, const char *buf, size_t count, loff_t *offset);
 ...
 * Ioctl
 */
int i2cdev_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg) {
	struct i2c_client *client = (struct i2c_client *) file->private_data;
	...
	case I2C_FUNCS:
		...
		return (copy_to_user((unsigned long *) arg, &funcs, sizeof(unsigned long))) ? -EFAULT : 0;
	case I2C_RDWR:
		if (copy_from_user(&rdwr_arg, (struct i2c_rdwr_ioctl_data *) arg, sizeof(rdwr_arg)))
			return -EFAULT;
		/* Put an arbitrary limit on the number of messages that can be sent at once */
		if (rdwr_arg.nmsgs > 42)
			return -EINVAL;
		rdwr_pa = (struct i2c_msg *) kmalloc(
				rdwr_arg.nmsgs * sizeof(struct i2c_msg), GFP_KERNEL);
		if (rdwr_pa == NULL)
			return -ENOMEM;
		...
		data_ptrs = (u8 **) kmalloc(rdwr_arg.nmsgs * sizeof(u8 *), GFP_KERNEL);
		if (data_ptrs == NULL) {
			kfree(rdwr_pa);
			return -ENOMEM;
		}
		...
		return res;
	case I2C_SMBUS:
		if (copy_from_user(&data_arg, (struct i2c_smbus_ioctl_data *) arg, sizeof(struct i2c_smbus_ioctl_data)))
			return -EFAULT;
		...
		if ((data_arg.size == I2C_SMBUS_QUICK)
			...
			return i2c_smbus_xfer(client->adapter, client->addr, client->flags, data_arg.read_write, data_arg.command, data_arg.size, NULL);
...
int i2cdev_attach_adapter(struct i2c_adapter *adap) {
	...
	if (!i2cdev_adaps[i]) {
		i2cdev_adaps[i] = adap;
#ifdef CONFIG_DEVFS_FS
		devfs_i2c[i] = devfs_register (devfs_handle, name, DEVFS_FL_DEFAULT, I2C_MAJOR, i, S_IFCHR | S_IRUSR | S_IWUSR, &i2cdev_fops, adap);
#endif
